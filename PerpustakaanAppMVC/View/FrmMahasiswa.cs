...
namespace PerpustakaanAppMVC.View
{
    public partial class FrmMahasiswa : Form
    {
        ...
        private void btnCari_Click(object sender, EventArgs e)
        {
            ...
            string namaCari = txtNama.Text;

            // Lakukan pencarian berdasarkan nama
            List<Mahasiswa> hasilPencarian = SearchByNama(namaCari);

            // Tampilkan hasil pencarian di ListView
            TampilkanHasilPencarian(hasilPencarian);
        }

        // Metode untuk menampilkan hasil pencarian di ListView
        private void TampilkanHasilPencarian(List<Mahasiswa> hasilPencarian)
        {
            // Kosongkan listview
            lvwMahasiswa.Items.Clear();
        ...
        // Metode pencarian berdasarkan nama di controller
        public List<Mahasiswa> SearchByNama(string keyword)
        {
            // Mencari mahasiswa berdasarkan nama menggunakan LINQ
            return listOfMahasiswa.Where(m => m.Nama.IndexOf(keyword, StringComparison.OrdinalIgnoreCase) >= 0).ToList();
        }
    }
}
